...
for (i in c(1:100)){
  ...
  C1<-quantile(abs(y_train),.95)
  
  t1<-Sys.time()
  
  GeneralSVR<-svm(x_train,y_train,type='eps-regression',kernel="radial",scale=FALSE, epsilon=0, cost=C1)
  ...
  ModelGeneral1<-svm(x_train,y_train, type='eps-regression', kernel = "radial", epsilon=0)
  ...
  A<-c(Scale,RobustEps, RMAE, GMAE, MaMAE, CVMAE, RRMSE,  GRMSE,  MaRMSE, CVRMSE, RobustT, CVT)
